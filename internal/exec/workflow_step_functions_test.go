...
func TestResolveWorkflowStepFunctions(t *testing.T) {
	...
	// Step 1: choose - all interactive fields resolved.
	...
	// Step 2: input - unset env falls back to the provided default.
...
func TestResolveWorkflowStepCommand(t *testing.T) {
	// The resolver reads the shared step executor state; reset it around the test
	// so it does not leak into (or inherit from) other workflow tests.
	...
	t.Run("resolves prior step results and step env", func(t *testing.T) {
