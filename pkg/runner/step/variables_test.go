...
func TestVariablesResolveWith(t *testing.T) {
	...
	t.Run("overlay wins over a persisted env value for the call only", func(t *testing.T) {
		...
		// The persisted value is unchanged after the call.
		assert.Equal(t, "use1", vars.Env["REGION"])
	})

	t.Run("nil overlay resolves against the persisted env", func(t *testing.T) {
		vars := NewVariables()
